...
Главное правило: не делать слепую массовую замену по всему проекту. Сначала менять данные в `src/data`, `public`, `index.html` и переменных окружения. Тесты, mock-данные и fixtures не менять без отдельной причины.
